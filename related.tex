...
Several works have done in identification of malicious packages within the \acp{NPM} ecosystem, 
but none of them considered the evolution of the code with respect to all the published releases.

One such tool is DONAPI \cite{huang2024donapimaliciousnpmpackages}, which used 
a combination of static analysis, based on a predefined database of sensitive \acp{APIs}; 
and dynamic analysis, to extract behavioral patterns in order to detect and classify malicious packages.

An alternative tool is SpiderScan \cite{10.1145/3691620.3695492},
which is based on graph-based behavior modeling and matching.
It uses \acp{LLM} for various tasks, including to identification 
of sensitive \acp{APIs} and the analysis of shell commands. 

Other tools also use \acp{LLM} for \acp{NPM} package analysis,
such as the tool by \textcite{zahan2025leveraginglargelanguagemodels, article, Zhang_2025}.

Although these tools achieve good results in identifying malicious packages updates, 
they are not open source and they also have limitations.

The dynamic analysis in DONAPI, according to \textcite{10.1145/3691620.3695492}, risks ``hindering their 
practical usefulness in real-world scenarios``.

Additionally a common issue, is that most existing tools
do not model malicious behavior in a holistic way. 
In fact, these tools often focus only on predefined set of discrete features,
like sensitive \acp{APIs}, code structures or semantics (e.g., data flow) \cite{10.1145/3691620.3695492}; but not considering 
the history of the package code evolution through all its releases.
These tools also require a huge amount of manual validation due to high number of false positives.

Furthermore, as discussed by \textcite{10.1145/3691620.3695492}, ``Prompt engineering methods 
leverage the capability of ChatGPT, and use self-refinement and chain-of-thought techniques to detect 
maliciousness, but can incur high expense in real-world scenarios``, besides the fact that in
general \acp{LLM} can be prone to hallucination, as he states \textcite{li2024surveyhonestylargelanguage}.
