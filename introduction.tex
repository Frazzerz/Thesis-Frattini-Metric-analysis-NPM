...
The open-source software \acp{OSS} ecosystem represents one of the fundamental 
pillars of modern software development, with millions of projects relying on publicly shared libraries and packages.

Within this context, the Node Package Manager \acp{NPM} ecosystem
for JavaScript emerges as one of the largest and most dynamic software repositories, 
hosting more than two million packages \cite{10.1145/3510457.3513044}.
...
when an attacker compromised 18 popular \acp{NPM} packages, including chalk and color \cite{henig2025breakdownwidespreadnpmsupplychainattack}.
These packages alone account for over 2.6 billion downloads each week,
making this attack one of the most impactful in the recent history of the \acp{NPM} ecosystem.
...
The goal is to evaluate whether it is possible to identify anomalous updates in an \acp{NPM} package
...
First, two real attacks that occurred within the \acp{NPM} ecosystem were selected
...
across all versions of an \acp{NPM} package.
...
In particular, 500 \acp{NPM} packages selected from the top 1000 most popular ones were analyzed,
...
most downloaded \acp{NPM} real-world packages, to evaluate its overall effectiveness.

%... The results obtained demonstrate the potential of this approach...
