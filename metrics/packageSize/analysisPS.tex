...
The packages exhibiting this trend are: \texttt{typescript}, \texttt{next}, and \texttt{aws-sdk}.
...
In the first attack, the attacker inserted a malicious \ac{DLL} weighing 1.2 MB into the tarball (\cref{sec:attackA}),
...
Attach B (\cref{sec:attackB}) increases the size of existing \texttt{index.js} file by adding a malicious line of 76,438 characters.
...
The attack C (\cref{sec:attackC}) which adds a malicious file weighing approximately 3.7 MB,
succeeds in placeing the compromised versions of the packages affected by this attack among the outlier values, 
...
In summary, the metric is efficient in detecting the attack C (Shai-Hulud~1.0 \cref{sec:analysisPackageSizeAttackC}) 
and the attack D (Shai-Hulud~2.0 \cref{sec:analysisPackageSizeAttackD})
