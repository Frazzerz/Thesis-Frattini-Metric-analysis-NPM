...
while Attack~C and D introduce files containing lines of 3,734,355 and 10,157,373 characters, respectively.
...
To verify this, we first characterized the distribution of this metric in the Goodware dataset,
...
where here we can observe that the median remains stable at approximately 460 characters, 
...
because an advertiser could inject a malicious code anywhere within the tarball.
...
from the Goodware dataset we selected three packages for individual analysis as representative cases, 
...
It shows the most extreme outlier in the entire Goodware dataset, 
...
consistently around 3,000,000 characters, making it the second most extreme outlier value in the Goodware dataset after \texttt{next}.
...
Before version 3.6.0, this package is within the confidence intervals, 
...
consists of 10 lines with the longest being 2,188,784 characters, making it the longest line found across all plain-text files in the package.
...
The reason is that the Goodware baseline includes packages with build artifacts containing lines of several tens of thousands in length, 
...
As shown in \cref{fig:fig2LLLAttackC}, this places the compromised versions among the outlier values of the Goodware distribution.
...
the outlier range but far exceed the maximum outlier values in the Goodware dataset.
...
remains within the high confidence interval bound established in the Goodware dataset.