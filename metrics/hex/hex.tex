...
\textbf{crypto-js} (solid green line) is a package related to cryptography, 
and implements cryptographic primitives, such as encryption algorithms and digital signatures~\cite{crypto-js}.
...
As shown at point~1 in \cref{fig:fig1HexIncreasing},
in version 3.1.2-3 the developers converted these values to hexadecimal,
aligning with standard cryptographic specification practices, such as the Secure Hash Algorithm (SHA)~\cite{gallagher1995secure}.
This brought the count to 546, an outlier maintained in subsequent versions.
A further jump at point~2 corresponds to the introduction of the Blowfish algorithm in version 4.2.0, 
...
As seen at point~3 in \cref{fig:fig1HexIncreasing}, 
it jumped from 0 to 2,208 in version 4.0.0-beta.2, the most extreme outlier for four consecutive releases.
This happened because the developers accidentally included a \texttt{.yarn} cache directory contained numerous files, including a build-tool-generated JavaScript file,
...
\textbf{xml2js} (solid blue line) went from~0 to~96 unique hexadecimal values at point~4 in \cref{fig:fig1HexIncreasing}.
...
\textbf{buffer-crc32} (solid green line) implements the Cyclic Redundancy Check (CRC32) algorithm for data integrity checking~\cite{buffer-crc32}.
For efficient reasons, the algorithm uses a CRC table, 
...
Until version 0.2.13, the CRC table value was stored as hexadecimal constants.
As shown at point~1 in \cref{fig:fig1HexDecreasing}, 
...
At point~2 in \cref{fig:fig1HexDecreasing}, 
starting from version 1.1.7, the test file was excluded from the published tarball for space optimization, 
and as it is not essential for its functionality and not needed by end users.
...
across all releases, always representing an outlier;
while \texttt{tweetnacl} (solid blue line) has 177, representing an outlier in the most.

\textbf{chardet} (solid pink line) detects character encoding using byte maps stored as 
hexadecimal constants in \texttt{/lib/encoding/sbcs.js}~\cite{chardet}, resulting 972 values.
...
\textbf{Attack~A} (Malicious Windows DLL, \cref{sec:attackA}) do not introduce any new hexadecimal values,
...
As shown in \cref{fig:fig2HexAttackB}, this positions the malicious versions among the outlier of the Goodware distribution, 
...
This outcome is consistent with the nature of the \ac{SSCA} as does not rely on any obfuscation technique, 
but rather on the presence of a clear-text malicious code.
