...
Understanding these cases is essential to distinguish legitimate anomalies from malicious injections when evaluating the \emph{Malware} dataset.
...
As shown in \cref{fig:fig2HexAttackB}, this positions the malicious versions among the outliers of the \emph{Goodware} distribution, 
...
releases within the confidence intervals of the \emph{Goodware} dataset, 
...
As shown in \cref{fig:fig2HexAttackD}, the count jumps to 122,535, far exceeding the maximum outlier values observed in the \emph{Goodware} dataset.
...
while Attack~D produces 122,535, an extreme anomaly with no parallel in the \emph{Goodware} baseline.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{metrics/hex/hex_plot2single_AttackB.pdf}
