...
The Malicious Windows \ac{DLL} attack (Attack~A, \cref{sec:attackA}) 
introduced a malicious Windows \ac{DLL} to achieve remote code execution on victim machines, 
the Crypto Attack (Attack~B, \cref{sec:attackB}) 
embedded obfuscated code in a popular package to hijack cryptocurrency transactions in the browser, 
and \texttt{Shai-Hulud}~1.0 and~2.0 (Attacks~C and~D, \cref{sec:attackC,sec:attackD}), 
the two variants of the \texttt{Shai-Hulud} worm, combined credential exfiltration and self-propagation across the registry.
...
a \emph{Goodware} dataset of 629 popular samples across their 20 most recent versions each,
...
because the introduction of a Windows \ac{PE} binary is essentially absent from the \emph{Goodware} baseline.
...
producing counts of 455 and 122,535 unique values respectively, far above the \emph{Goodware} confidence intervals.
...
Expanding the \emph{Malware} dataset to cover a broader range of \ac{SSCA} families would also strengthen future evaluations
